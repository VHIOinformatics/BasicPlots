...
#' @param ... Additional parameters to be added to autoplot
#'
#' @import ggplot2
#' @import ggfortify
#' @import plotly
...
makePCA <- function(counts, targets, PCs = NULL, label = FALSE, title = NULL, color_condition = NULL, shape_condition = NULL, colors = NULL, resultsDir = getwd(), fileName = "PCAPlot", fmtPlot = "", interactive = FALSE, wid = 8, hei = 6, res = 400, ...) {
  ...
  if(is.null(colors)) {
    colors <- c("#288A6C", "#CF7937", "#8682BB", "#BB4F86", "#E3BC51","#8CB369", "#A6CEE3","#F1AEAD", "#7C7B7B","#1F78B4", "#D8BFD8", "#854849" ,"#5D7344","#10438B", "#FDBF6F", "#61A175", "#BB4042", "#F18684", "#5C3F7C", "#D0A376", "#EDED90","#B2DF8A")
  ...
  if(fmtPlot == "") {
    ...
    ggsave(file.path(resultsDir, paste0(fileName, ".", fmtPlot)), plot = p, width = wid, height = hei, dpi = res)
